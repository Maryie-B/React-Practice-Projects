...
const Filters = () => {
  const {filters: {text, category, company, color, min_price, price, max_price, shipping}, clearFilters, updateFilters, all_products } = useFilterContext();
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="form-control">
          ...
          <div className="colors">
            {colors.map((c, index) => {
              if (c === 'all') {
                return <button key={index} name='color' onClick={updateFilters} data-color='all' className={`${color === 'all' ? 'all-btn active' : 'all-btn' }`}></button>
              }
